...
public abstract class JmeConfigurations {
	...
	public static Material createLightingMaterial(AssetManager assetManager, ColorRGBA color) {
		...
		setAmbientColor(mat, color);
		setDiffuseColor(mat, color);
		// specular color: green with alpha (transparency)
		setSpecularColor(mat, ColorRGBA.fromRGBA255(0, 255, 0, 150));
	...
	public static void changeAlpha(Node geo, ColorRGBA color, int deltaAlpha) {

		// adapt color to transparency value
		color.a = (150f + deltaAlpha) / 255;

		for (int i = 0; i < geo.getChildren().size(); i++) {
			Geometry geo1 = (Geometry) geo.getChild(i);

			// set colors of this geometry
			setAmbientColor(geo1.getMaterial(), color);
			setDiffuseColor(geo1.getMaterial(), color);
			// specular color: green with alpha (transparency)
			setSpecularColor(geo1.getMaterial(), ColorRGBA.fromRGBA255(0, 255, 0, (150 + deltaAlpha) / 255));
		}

	}

	public static void changeAlpha(Geometry geo, ColorRGBA color, int deltaAlpha) {

		// adapt color to transparency value
		color.a = (150f + deltaAlpha) / 255;

		// set colors of this geometry
		setAmbientColor(geo.getMaterial(), color);
		setDiffuseColor(geo.getMaterial(), color);
		// specular color: green with alpha (transparency)
		setSpecularColor(geo.getMaterial(), ColorRGBA.fromRGBA255(0, 255, 0, (150 + deltaAlpha) / 255));
	}

	private static void setAmbientColor(Material mat, ColorRGBA color) {
		mat.setColor("Ambient", color);
	}

	private static void setDiffuseColor(Material mat, ColorRGBA color) {
		mat.setColor("Diffuse", color);
	}

	private static void setSpecularColor(Material mat, ColorRGBA color) {
